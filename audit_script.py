...
try:
    with open('nightly_processor.py', 'r') as f:
        content = f.read()
    if "cursor" not in content and "limit" not in content and "tail" not in content:
        add_status("S1-7", "Unbounded Growth in Nightly Appends", "STILL PRESENT", "nightly_processor.py")
    else:
        add_status("S1-7", "Unbounded Growth in Nightly Appends", "FIXED", "nightly_processor.py")
except:
    pass
...
try:
    ...
    if "git commit" in content and "git status --porcelain" not in content:
        add_status("S1-10", "Fragile CI/CD Pipeline Logic", "STILL PRESENT", "nightly_processor.py")
    else:
        add_status("S1-10", "Fragile CI/CD Pipeline Logic", "FIXED", "nightly_processor.py")
except:
    pass
...
try:
    ...
    if "8534649457" in content:
        add_status("S1-14", "Hardcoded Chat ID & Config Drift", "STILL PRESENT", "nightly_processor.py")
    else:
        add_status("S1-14", "Hardcoded Chat ID & Config Drift", "FIXED", "nightly_processor.py")
except:
    pass
