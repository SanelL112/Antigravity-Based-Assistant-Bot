...
def enforce_all_rotations():
    ...
    history_dirs = {CHAT_HISTORY_DIR, BASE_DIR}
    seen_history_files = set()
    for d in history_dirs:
        for f in d.glob("chat_history_*.txt"):
            if f in seen_history_files:
                continue
            seen_history_files.add(f)
            try:
                os.chmod(f, 0o600)
            except OSError:
...
def _is_command_allowed(cmd: str) -> tuple[bool, str]:
    ...
    # Fixed diagnostic actions (e.g. health, uptime, memory, disk, services, ollama)
    action = _diagnostic_action(cmd_stripped)
    ...
    import config
    safe_roots = [Path(root).resolve() for root in getattr(config, "SAFE_BASH_ROOTS", [])]
...
def run_bash_safely(cmd: str, chat_id: int = 0, timeout: int = 60) -> str:
    ...
    if action in _DIAGNOSTIC_ACTIONS:
        ...
        try:
            ...
            logger.warning("Command execution failed: %s", type(exc).__name__)
