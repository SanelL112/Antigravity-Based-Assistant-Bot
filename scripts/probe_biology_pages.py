...
def main() -> int:
    ...
    # Cold SharePoint loads take >40s before ANY rail element renders —
    # the daemon waits the same 60s before expanding.
    deadline = time.monotonic() + 60
    while time.monotonic() < deadline:
        if js(driver,
              "return document.querySelectorAll('.sectionListItem, "
              "[aria-label*=\"Section Group\"]').length;"):
            break
        time.sleep(3)
    ...
    while time.monotonic() < deadline:
        ...
        if n == 0:
            break
        time.sleep(3)
    # Expansion re-renders the rail; wait for inner sections to mount
    # (daemon gives this 30s before enumerating).
    deadline = time.monotonic() + 30
    while time.monotonic() < deadline:
