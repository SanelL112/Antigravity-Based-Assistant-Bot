...
def collect_sources() -> list[dict]:
    ...
    sc_dir = CACHE_DIR
    legacy_sc_dirs = [os.path.join(BASE_DIR, "cache"), os.path.join(BASE_DIR, "scrapers", "source_cache")]
    for d in [sc_dir] + legacy_sc_dirs:
        if os.path.isdir(d):
            if d in legacy_sc_dirs and d != sc_dir:
                logger.warning(f"Reading stale legacy cache from {d}")
