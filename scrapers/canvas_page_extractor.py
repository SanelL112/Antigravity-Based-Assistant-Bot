...
def _clean_task_title(title: str, max_len: int = 55) -> str:
    ...
    # Junk/prose rejection runs on the FULL row, before length truncation:
    # truncation can cut a row right before the phrase that identifies it as
    # prose (", so describe ..."), and the gate never truncates, so deciding
    # here first is what keeps the two filters in agreement.
    core_full = t.rstrip(":;-–—. ").strip()
    if (
        not core_full
        or _JUNK_TITLE_RE.fullmatch(core_full)
        or re.fullmatch(r"[\d\s./&-]+", core_full)
        or _DATE_HEADER_RE.fullmatch(core_full)
        or re.search(r"&#|#[xX][0-9a-fA-F]{1,4};", core_full)
        or _prose_title_reject(core_full)
    ):
        return ""

    # If title is still too long, truncate cleanly at word boundary
    if len(core_full) > max_len:
        core_full = core_full[:max_len].rsplit(" ", 1)[0].rstrip(" ,.-:").strip()
...
# Question openers are note-page fragments ("Which type of bond creates...").
...
def _prose_title_reject(core: str) -> bool:
    ...
    Mirror of the gate-side filter in ``assignment_calendar`` (kept in sync by
    hand — importing it back would create a circular import). These rows
    arrive from syllabi ("Summative = 75%"), OneNote note pages ("Main ideas
    / Questions for class:"), and lesson text ("In order for theorems to be
    properly applied, we need..."). None are actionable tasks.
    """
    if "%" in core:
        return True
    ...
    if len(core.split()) <= 3 and re.match(r"^the\b", core, re.IGNORECASE):
        return True
    # Mangled multi-item agenda row ("... - 1) do X 2) do Y") — the split
    # above keeps only the first item, so a surviving two-item row is a
    # stale duplicate of a cleaner variant elsewhere.
