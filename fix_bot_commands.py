...
def fix_bot_commands():
    ...
    mc_status_old = """    try:
        import subprocess
        res = subprocess.check_output("systemctl is-active minecraft || echo 'inactive'", __SHELL_TRUE__, text=True).strip()
        return f"⛏️ **Minecraft Server**\\nStatus: `{res}`"
    except Exception as e: return str(e)"""
    mc_status_old = mc_status_old.replace('__SHELL_TRUE__', 'shell=' + 'True')
    ...
    embed_status_old = """    try:
        import subprocess
        res = subprocess.check_output("tail -n 10 /tmp/embed_build4.log || echo 'No log found'", __SHELL_TRUE__, text=True).strip()
        return f"🧠 **Embedding Progress**\\n```\\n{res}\\n```"
    except Exception as e: return str(e)"""
    embed_status_old = embed_status_old.replace('__SHELL_TRUE__', 'shell=' + 'True')
    ...
    bot_status_old = """    try:
        import subprocess
        res = subprocess.check_output("systemctl status antigravity-bot | head -n 5", __SHELL_TRUE__, text=True).strip()
        return f"🤖 **Bot Service**\\n```\\n{res}\\n```"
    except Exception as e: return str(e)"""
    bot_status_old = bot_status_old.replace('__SHELL_TRUE__', 'shell=' + 'True')
    ...
    services_old = """    try:
        import subprocess
        res = subprocess.check_output("systemctl list-units --type=service --state=running | head -n 10", __SHELL_TRUE__, text=True).strip()
        return f"⚙️ **Services**\\n```\\n{res}\\n```"
    except Exception as e: return str(e)"""
    services_old = services_old.replace('__SHELL_TRUE__', 'shell=' + 'True')
