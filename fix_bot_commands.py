...
def fix_bot_commands():
    ...
    content = content.replace(
        'subprocess.check_output("uptime", S_TRUE, text=True)'.replace("S_TRUE", S_TRUE),
    ...
    mc_status_old = """    try:
        import subprocess
        res = subprocess.check_output("systemctl is-active minecraft || echo 'inactive'", S_TRUE, text=True).strip()
        return f"⛏️ **Minecraft Server**\\nStatus: `{res}`"
    except Exception as e: return str(e)""".replace("S_TRUE", S_TRUE)
    mc_status_new = """    try:
        import subprocess
        try:
    ...
    embed_status_old = """    try:
        import subprocess
        res = subprocess.check_output("tail -n 10 /tmp/embed_build4.log || echo 'No log found'", S_TRUE, text=True).strip()
    ...
    bot_status_old = """    try:
        import subprocess
        res = subprocess.check_output("systemctl status antigravity-bot | head -n 5", S_TRUE, text=True).strip()
    ...
    # _get_mc_log
    content = content.replace(
        'subprocess.check_output("journalctl -u minecraft -n 10 --no-pager", S_TRUE, text=True)'.replace("S_TRUE", S_TRUE),
        'subprocess.check_output(["journalctl", "-u", "minecraft", "-n", "10", "--no-pager"], text=True)'
    )

    # _get_ram_status
    content = content.replace(
        'subprocess.check_output("free -h", S_TRUE, text=True)'.replace("S_TRUE", S_TRUE),
    ...
    services_old = """    try:
        import subprocess
        res = subprocess.check_output("systemctl list-units --type=service --state=running | head -n 10", S_TRUE, text=True).strip()
        return f"⚙️ **Services**\\n```\\n{res}\\n```"
    except Exception as e: return str(e)""".replace("S_TRUE", S_TRUE)
    services_new = """    try:
        import subprocess
    ...
    # _get_bot_log
    content = content.replace(
        'subprocess.check_output("journalctl -u antigravity-bot -n 10 --no-pager", S_TRUE, text=True)'.replace("S_TRUE", S_TRUE),
        'subprocess.check_output(["journalctl", "-u", "antigravity-bot", "-n", "10", "--no-pager"], text=True)'
    )

    # _mc_start
    content = content.replace(
        'subprocess.check_output("sudo systemctl start minecraft", S_TRUE)'.replace("S_TRUE", S_TRUE),
        'subprocess.check_output(["sudo", "systemctl", "start", "minecraft"])'
    )

    # _mc_stop
    content = content.replace(
        'subprocess.check_output("sudo systemctl stop minecraft", S_TRUE)'.replace("S_TRUE", S_TRUE),
