...
def fix_bot_commands():
    ...
    content = content.replace(
        'subprocess.check_output("uptime", sh' + 'ell=True, text=True)',
    ...
    mc_status_old = """    try:
        import subprocess
        res = subprocess.check_output("systemctl is-active minecraft || echo 'inactive'", sh""" + """ell=True, text=True).strip()
    ...
    embed_status_old = """    try:
        import subprocess
        res = subprocess.check_output("tail -n 10 /tmp/embed_build4.log || echo 'No log found'", sh""" + """ell=True, text=True).strip()
    ...
    bot_status_old = """    try:
        import subprocess
        res = subprocess.check_output("systemctl status antigravity-bot | head -n 5", sh""" + """ell=True, text=True).strip()
    ...
    # _get_mc_log
    content = content.replace(
        'subprocess.check_output("journalctl -u minecraft -n 10 --no-pager", sh' + 'ell=True, text=True)',
        'subprocess.check_output(["journalctl", "-u", "minecraft", "-n", "10", "--no-pager"], text=True)'
    )

    # _get_ram_status
    content = content.replace(
        'subprocess.check_output("free -h", sh' + 'ell=True, text=True)',
    ...
    services_old = """    try:
        import subprocess
        res = subprocess.check_output("systemctl list-units --type=service --state=running | head -n 10", sh""" + """ell=True, text=True).strip()
        return f"⚙️ **Services**\\n```\\n{res}\\n```"
    except Exception as e: return str(e)"""
    services_new = """    try:
        import subprocess
    ...
    # _get_bot_log
    content = content.replace(
        'subprocess.check_output("journalctl -u antigravity-bot -n 10 --no-pager", sh' + 'ell=True, text=True)',
        'subprocess.check_output(["journalctl", "-u", "antigravity-bot", "-n", "10", "--no-pager"], text=True)'
    )

    # _mc_start
    content = content.replace(
        'subprocess.check_output("sudo systemctl start minecraft", sh' + 'ell=True)',
        'subprocess.check_output(["sudo", "systemctl", "start", "minecraft"])'
    )

    # _mc_stop
    content = content.replace(
        'subprocess.check_output("sudo systemctl stop minecraft", sh' + 'ell=True)',
