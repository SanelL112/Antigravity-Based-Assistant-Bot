...
def fix_bot_commands():
    ...
    content = content.replace(
        'subprocess.check_output("uptime", shell=True, text=True)',  # nosec B602 B604 B605 B607
...
"""    try:
    ...
    # _get_mc_log
    content = content.replace(
        'subprocess.check_output("journalctl -u minecraft -n 10 --no-pager", shell=True, text=True)',  # nosec B602 B604 B605 B607
        'subprocess.check_output(["journalctl", "-u", "minecraft", "-n", "10", "--no-pager"], text=True)'
    )

    # _get_ram_status
    content = content.replace(
        'subprocess.check_output("free -h", shell=True, text=True)',  # nosec B602 B604 B605 B607
...
"""    try:
    ...
    # _get_bot_log
    content = content.replace(
        'subprocess.check_output("journalctl -u antigravity-bot -n 10 --no-pager", shell=True, text=True)',  # nosec B602 B604 B605 B607
        'subprocess.check_output(["journalctl", "-u", "antigravity-bot", "-n", "10", "--no-pager"], text=True)'
    )

    # _mc_start
    content = content.replace(
        'subprocess.check_output("sudo systemctl start minecraft", shell=True)',  # nosec B602 B604 B605 B607
        'subprocess.check_output(["sudo", "systemctl", "start", "minecraft"])'
    )

    # _mc_stop
    content = content.replace(
        'subprocess.check_output("sudo systemctl stop minecraft", shell=True)',  # nosec B602 B604 B605 B607
